import * as Legacy from './universal-engine.js';
export * from './universal-engine.js';
...
export function buildArtifact(project, prompt = '') {
  const wantsGame = project?.kind === 'game' || /\b(flappy|game|playable|platformer|arcade|pong|tetris)\b/i.test(String(project?.intent || prompt));
  if (!wantsGame) return Legacy.buildArtifact(project, prompt);
...
export function sandboxRun(project) {
  ...
  const game = String(project?.artifacts?.['game.js'] || '');
  const checks = [];
  if (project?.kind === 'game' || game) {
    ...
    checks.push({name:'game-classic-script',status:/\bexport\s+(?:default\s+)?function/.test(game)?'failed':'passed'});
  }
...
export function browserTest(project, runner = null) {
  ...
  return sandboxRun(project).runtimeChecks?.map(x=>({...x,verificationMode:'static-runtime-contract'})) || [];
}

export function runSyntheticUsers(project, users = Legacy.syntheticUsers(project), runner = null) {
  return users.map(user => {
    if (typeof runner !== 'function') return {...user,completed:false,errors:['real-synthetic-runner-required']};
    try { const r=runner(project,user)||{}; return {...user,completed:Boolean(r.completed),errors:r.errors||[]}; }
    catch(e){ return {...user,completed:false,errors:[String(e)]}; }
  });
}

export function createDeploymentAdapter(target='web') {
  return {target,configured:false,async deploy(){return{status:'unavailable',reason:'deployment-adapter-not-configured'}}};
}
export async function deploy(project,adapter,target='web') {
  const verification=typeof Legacy.verify==='function'?Legacy.verify(project):{passed:false};
  if(!verification.passed)return{status:'blocked',reason:'verification-failed',tests:verification.tests||[]};
  const a=adapter||createDeploymentAdapter(target);
  return a.deploy({project,target});
}
...
export function runUniversalSimulation(intent='Build something useful') {
  const result=Legacy.runUniversalSimulation(intent);
  return {...result,compiled:{...(result.compiled||{}),status:'simulated'},cloud:{...(result.cloud||{}),status:'simulated'},deployed:{...(result.deployed||{}),status:'simulated'},simulationOnly:true};
}
