...
async function edge(action, payload = {}) {
  ...
  const url = `${window.BUILDER_CONFIG.SUPABASE_URL}/functions/v1/ai`;
  const res = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${session.access_token}` }, body: JSON.stringify({ action, ...payload }) });
  const data = await res.json().catch(() => ({}));
  if (!res.ok || data?.ok === false) throw new Error(data?.error || `AI service error (${res.status})`);
  return data;
}
...
function shell(body, active='home'){const root=ensureShell();const recents=state.projects.slice(0,6);root.innerHTML=`<aside class="side"><div class="logo">ProjectX</div><button class="new" data-nav="home">+ New creation</button><nav class="nav"><button data-nav="home" class="${active==='home'?'active':''}">⌂ <span>Home</span></button><button data-nav="projects" class="${active==='projects'?'active':''}">□ <span>Projects</span></button><button data-nav="analytics" class="${active==='analytics'?'active':''}">▥ <span>Analytics</span></button><button data-nav="settings" class="${active==='settings'?'active':''}">⚙ <span>Settings</span></button></nav><div class="divider"></div><div class="label">Assistant</div><button class="assist-btn" data-nav="assistant">✦ Assistant X</button><div class="divider"></div><div class="label">Recent</div><div class="recent">${recents.map(p=>`<button data-open="${esc(p.id)}">${esc(p.title)}</button>`).join('')||'<div class="sub" style="padding:6px 10px">No projects yet</div>'}</div><div class="acct">${session?.user?.email?`Signed in as ${esc(session.user.email)}`:'Guest workspace'}<div class="sub" style="font-size:10px;margin-top:2px">${session?'Cloud sync enabled':'Local sync enabled'}</div></div></aside><main class="main"><div class="top"><button data-nav="home">Home</button>${session?'<button data-action="signout">Sign out</button>':'<button data-action="signin">Sign in</button>'}</div>${body}</main>`;root.onclick=async e=>{const nav=e.target.closest('[data-nav]')?.dataset.nav;if(nav)return navigate(nav);const open=e.target.closest('[data-open]')?.dataset.open;if(open)return openProject(open);const action=e.target.closest('[data-action]')?.dataset.action;if(action==='signin')return authModal();if(action==='signout'){await signOut();home();}};return root;}
function home(){shell(`<div class="wrap"><div class="center"><div class="kicker">PROJECT X</div><h1 class="hero-title">What do you want to create?</h1><p class="sub" style="max-width:680px;margin:0 auto">Start with one conversation. ProjectX understands the goal, asks only what is missing, creates a project brain, and builds the workspace and output it actually needs.</p><div class="composer"><textarea id="start-input" placeholder="Tell me what you want to create..."></textarea><div class="composer-foot"><span class="sub">${session?'AI account connected':localGuestKey()?`Gemini connected ${guestStatus()?.hint||''}`:'AI key required to start'}</span><button class="send" id="start-send">→</button></div></div><div class="chips"><button class="chip" data-example="Build a mobile app for tracking study sessions">Study app</button><button class="chip" data-example="Create a browser game where a monkey avoids obstacles">Browser game</button><button class="chip" data-example="Research the EV charging market in India">Research</button><button class="chip" data-example="Plan a small cafe near a college campus">Business</button></div></div></div>`,'home');const input=$('#start-input');$$('.chip',$('#px-app')).forEach(b=>b.onclick=()=>{input.value=b.dataset.example;input.focus()});$('#start-send').onclick=()=>beginCreation(input.value);input.onkeydown=e=>{if((e.ctrlKey||e.metaKey)&&e.key==='Enter'){e.preventDefault();$('#start-send').click();}};}
async function beginCreation(text){const intent=String(text||'').trim();if(!intent)return;if(!session&&!localGuestKey())return aiRequiredModal('ProjectX needs an AI provider before it can understand the project.');const history=[{role:'user',text:intent}],meta={answers:[]};renderInterview(history,meta);await continueInterview(history,meta.answers);}
function renderInterview(history,meta){shell(`<div class="interview"><div class="kicker">PROJECT X · ONE CHAT</div><h1 class="hero-title" style="font-size:46px">Let’s understand it.</h1><p class="sub">Stay in this one conversation until the project is sufficiently understood.</p><div id="interview-log" class="conversation"></div><form id="interview-form" class="form"><textarea id="interview-input" placeholder="Answer in your own words..."></textarea><button id="interview-send">Send</button></form><div id="interview-status" class="sub" style="text-align:center;margin-top:8px">Thinking…</div></div>`,'home');drawConversation(history,'#interview-log');$('#interview-form').onsubmit=async e=>{e.preventDefault();const input=$('#interview-input'),send=$('#interview-send'),text=input.value.trim();if(!text||send.disabled)return;send.disabled=true;history.push({role:'user',text});meta.answers.push(text);input.value='';drawConversation(history,'#interview-log');await continueInterview(history,meta.answers);send.disabled=false;};$('#interview-input').onkeydown=e=>{if((e.ctrlKey||e.metaKey)&&e.key==='Enter'){e.preventDefault();$('#interview-form').requestSubmit();}};}
