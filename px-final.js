...
function renderInterviewUnderstanding(data){
  ...
  root.innerHTML=`<div class="understanding-main"><div class="understanding-copy"><div class="understanding-eyebrow">PROJECT BRIEF</div><div class="understanding-title">What ProjectX understands</div><div class="sub understanding-summary">${esc(summary||'Building the project brief from your request.')}</div>${category?`<div class="understanding-category">Focus · ${esc(category)}</div>`:''}${known.length?`<div class="understanding-known">${known.map(x=>`<span>${esc(x)}</span>`).join('')}</div>`:''}${missing.length?`<div class="understanding-meta"><span>Next: ${esc(missing.join(', '))}</span></div>`:''}</div></div>`;
}
function discoveryFallbackQuestion(data, quality, missing, project){
...
async function continueInterview(history, answers, meta={}){
  ...
  try{
    ...
    if(!done){
      const question=discoveryFallbackQuestion(data,quality,missing,mergedProject);
