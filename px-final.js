...
function renderBrain(project){
  ...
  const html=brainList('Understanding',rows)+brainList('Project context',context)+brainList('Requirements',s.requirements)+brainList('Constraints',s.constraints)+brainList('Decisions',s.decisions)+brainList('Deliverables',s.deliverables)+brainList('Success criteria',s.successCriteria)+brainList('Dependencies',s.dependencies)+brainList('Open questions',s.openQuestions)+brainList('Known resources',s.resources)+
...
function renderProjectChat(project,prefill=''){
  ...
  if(approve)approve.onclick=async()=>{
    ...
    try{
      ...
      const mutation=applyProjectMutation(project,{specPatch:pending.specPatch||{},workspaceSections:Array.isArray(pending.workspaceSections)?pending.workspaceSections:undefined,agents:Array.isArray(pending.agents)?pending.agents:undefined,fileOperations:Array.isArray(pending.fileOperations)?pending.fileOperations:[]});
  ...
  $('#project-form').onsubmit=async e=>{
    ...
    try{
      ...
      const wantsMutation=Boolean(data.changed||(data.specPatch&&typeof data.specPatch==='object'&&Object.keys(data.specPatch).length)||(Array.isArray(data.workspaceSections)&&data.workspaceSections.length)||(Array.isArray(data.fileOperations)&&data.fileOperations.length)||(Array.isArray(data.agents)&&data.agents.length));
      ...
      if(wantsMutation){
        ...
        const mutation=applyProjectMutation(project,{specPatch:data.specPatch||{},workspaceSections:Array.isArray(data.workspaceSections)?data.workspaceSections:undefined,agents:Array.isArray(data.agents)?data.agents:undefined,fileOperations:Array.isArray(data.fileOperations)?data.fileOperations:[]});
