...
else if(which==='appearance')body.innerHTML=`<h2>Appearance</h2><div class="box"><div class="row"><b>Theme</b><span class="sub">ProjectX workspace uses the same light editorial interface as the public product shell.</span></div></div>`;
...
else if(which==='billing')body.innerHTML=`<h2>Billing & Usage</h2><div class="box"><div class="row"><div><b>Payments</b><div class="sub">Payments are intentionally paused while ProjectX is being tested. Usage can still be inspected; no checkout is started from this workspace.</div></div></div><div class="grid" style="margin-top:10px"><div class="box"><b>30-day requests</b><div id="usage-total" class="hero-title" style="font-size:28px;margin:6px 0">—</div></div><div class="box"><b>30-day units</b><div id="usage-units" class="hero-title" style="font-size:28px;margin:6px 0">—</div></div><div class="box"><b>Top actions</b><div id="usage-actions" class="sub" style="margin-top:7px">Loading…</div></div></div><div class="box" style="margin-top:10px"><b>Recent AI usage</b><div id="usage-recent" style="margin-top:8px">Loading…</div></div></div>`;
...
function renderIntegrations(){
  toolShell('INTEGRATIONS','Connectors','Manage real service connections from Settings. ProjectX never treats a connector as connected until its backend handshake succeeds.','<div class="box"><div class="row"><div><b>GitHub</b><div class="sub">OAuth, repository browsing, and file pushes.</div></div><span class="status warn">CHECK IN SETTINGS</span></div><div class="row"><div><b>Supabase</b><div class="sub">'+(ensureSupabase()?'Browser client configured.':'Browser client not configured.')+'</div></div><span class="status '+(ensureSupabase()?'ok':'warn')+'">'+(ensureSupabase()?'READY':'SETUP NEEDED')+'</span></div><div class="row"><div><b>Deployment</b><div class="sub">Vercel and Netlify credentials are stored encrypted server-side.</div></div><button class="ghost" id="project-open-integrations">Manage</button></div><div class="row"><div><b>Collaboration</b><div class="sub">Members, invitations, and presence.</div></div><button class="ghost" id="project-open-members">Open Members</button></div></div>');
  $('#project-open-integrations')?.addEventListener('click',()=>settingsPage('integrations'));
  $('#project-open-members')?.addEventListener('click',()=>renderCollab(activeProject()));
}
